...
describe("PostgreSQL backup operations",()=>{
  it("runs a persistent backup service with a dedicated volume",()=>{const compose=read("docker-compose.yml");expect(compose).toContain("backup:");expect(compose).toContain("shop_agent_backups:/backups");expect(compose).toContain("BACKUP_RETENTION_DAYS");});
