...
async function selectStore(storeId) {
  ...
  try {
    const [configBody, overviewBody, usageBody,publicationBody,backupBody,privacyBody] = await Promise.all([api(`/v1/admin/stores/${encodeURIComponent(storeId)}/config`), api(`/v1/admin/stores/${encodeURIComponent(storeId)}/overview`),api(`/v1/admin/stores/${encodeURIComponent(storeId)}/usage`),api(`/v1/admin/stores/${encodeURIComponent(storeId)}/publication-readiness`),api("/v1/admin/system/backups"),api(`/v1/admin/stores/${encodeURIComponent(storeId)}/privacy`)]);
    state.config = structuredClone(configBody.config); state.overview = overviewBody.overview; state.usage=usageBody;state.publication=publicationBody.readiness;state.backup=backupBody;state.privacy=privacyBody; state.dirty = false;
...
function ensurePrivacy(){state.config.privacy||={conversationHistoryEnabled:true,conversationRetentionDays:90,allowAdminExport:true};return state.config.privacy}
function renderPrivacy(){if(!state.config)return;const privacy=ensurePrivacy(),stats=state.privacy?.statistics||{};[["privacy-history-enabled",String(privacy.conversationHistoryEnabled)],["privacy-retention-days",privacy.conversationRetentionDays],["privacy-export-enabled",String(privacy.allowAdminExport)],["privacy-notice-url",privacy.privacyNoticeUrl||""]].forEach(([id,value])=>$("#"+id).value=value);$("#privacy-metrics").innerHTML=[[stats.count||0,"Zapisane rozmowy"],[stats.expiring||0,"Wygasną w ciągu 7 dni"],[privacy.conversationRetentionDays,"Dni retencji"],[privacy.conversationHistoryEnabled?"Tak":"Nie","Nowe zapisy"]].map(([value,label])=>`<article class="metric"><strong>${value}</strong><small>${label}</small></article>`).join("");$("#privacy-audit-list").innerHTML=(state.privacy?.audit||[]).map(item=>`<div class="audit-row"><div><strong>${escapeHtml(item.action)}</strong><small>${new Date(item.createdAt).toLocaleString("pl-PL")} · ${escapeHtml(item.actor)}</small></div><code>${escapeHtml(item.targetId||item.targetType)}</code></div>`).join("")||'<p class="muted">Brak operacji administracyjnych.</p>';$("#export-conversations").disabled=!privacy.allowAdminExport}
async function loadPrivacy(){state.privacy=await api(`/v1/admin/stores/${encodeURIComponent(state.storeId)}/privacy`);renderPrivacy()}
...
[["privacy-history-enabled","conversationHistoryEnabled",v=>v==="true"],["privacy-retention-days","conversationRetentionDays",Number],["privacy-export-enabled","allowAdminExport",v=>v==="true"],["privacy-notice-url","privacyNoticeUrl",String]].forEach(([id,key,convert])=>$("#"+id).addEventListener("change",event=>{const value=convert(event.target.value);if(key==="privacyNoticeUrl"&&!value)delete ensurePrivacy()[key];else ensurePrivacy()[key]=value;markDirty()}));
$("#refresh-privacy").addEventListener("click",()=>loadPrivacy().catch(error=>toast(error.message,true)));
$("#purge-conversations").addEventListener("click",async()=>{const result=await api(`/v1/admin/stores/${state.storeId}/privacy/purge`,{method:"POST"});toast(`Usunięto ${result.deleted} rozmów po terminie.`);await loadPrivacy();state.conversations=null});
$("#delete-all-conversations").addEventListener("click",event=>confirmInline(event.currentTarget,"Potwierdź usunięcie",async()=>{const result=await api(`/v1/admin/stores/${state.storeId}/conversations`,{method:"DELETE",body:JSON.stringify({confirmation:`DELETE-${state.storeId}`})});toast(`Usunięto ${result.deleted} rozmów.`);state.conversations=null;renderConversations();await loadPrivacy()}));
$("#export-conversations").addEventListener("click",async()=>{const response=await fetch(`/v1/admin/stores/${encodeURIComponent(state.storeId)}/conversations-export`,{credentials:"same-origin"});if(!response.ok){const body=await response.json().catch(()=>({}));return toast(body.error||"Eksport nie powiódł się.",true)}const blob=await response.blob(),url=URL.createObjectURL(blob),link=document.createElement("a");link.href=url;link.download=`${state.storeId}-conversations.json`;link.click();URL.revokeObjectURL(url);toast("Eksport został pobrany.");await loadPrivacy()});
...
$("#conversations-list").addEventListener("click", async (event) => {
  ...
  const deleteButton=event.target.closest("[data-delete-conversation]");if(deleteButton){event.preventDefault();event.stopPropagation();const card=deleteButton.closest("[data-conversation-id]");confirmInline(deleteButton,"Potwierdź",async()=>{await api(`/v1/admin/stores/${state.storeId}/conversations/${card.dataset.conversationId}`,{method:"DELETE",body:JSON.stringify({confirmation:card.dataset.conversationId})});await loadConversations();await loadPrivacy();toast("Rozmowa została usunięta.")})}
});
