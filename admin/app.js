const state = { stores: [], storeId: "", config: null, overview: null, usage: null, publication: null, analysis: null, conversations: null, quality: null, syncJobs: null, onboarding: { feed: null, page: null, stage: 1 }, dirty: false, view: "overview" };
...
async function selectStore(storeId) {
  ...
  try {
    const [configBody, overviewBody, usageBody,publicationBody] = await Promise.all([api(`/v1/admin/stores/${encodeURIComponent(storeId)}/config`), api(`/v1/admin/stores/${encodeURIComponent(storeId)}/overview`),api(`/v1/admin/stores/${encodeURIComponent(storeId)}/usage`),api(`/v1/admin/stores/${encodeURIComponent(storeId)}/publication-readiness`)]);
    state.config = structuredClone(configBody.config); state.overview = overviewBody.overview; state.usage=usageBody;state.publication=publicationBody.readiness; state.dirty = false;
    renderAll(); setSaveState();
  } catch (error) { toast(error.message, true); }
}

function renderAll() {
  renderOverview(); renderGeneral(); renderUsage(); renderPublication(); renderGuided(); renderSyncSchedule(); renderSources(); renderTopics(); renderRules(); renderComparison(); renderWidget(); renderJson();
}
