...
export function buildComparisonConversationResponse(input: {
  ...
  return {
    ...
    state: input.state ?? { criteria: {} }, suggestions: selected.slice(0, 3).map((product) => ({ label: `Pokaż tańsze podobne do ${product.title}`, key: "similarCheaper" as const, value: product.externalId })),
