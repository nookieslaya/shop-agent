...
import { QualityAnalyticsRepository } from "../quality/analytics.js";
...
export async function createServer() {
  ...
  app.addHook("onResponse",async(request,reply)=>{const route=request.routeOptions.url;if(!route?.startsWith("/v1/")||route==="/v1/admin/session")return;const source={...((request.params&&typeof request.params==="object")?request.params:{}),...((request.body&&typeof request.body==="object")?request.body:{})} as Record<string,unknown>;const rawStore=source.storeId,storeId=typeof rawStore==="string"&&/^[a-z0-9][a-z0-9-]{0,79}$/i.test(rawStore)?rawStore:undefined;if(!storeId)return;const{db,close}=createDatabase();try{const config=(await new StoreConfigurationRepository(db).resolve(storeId))?.observability??defaultObservability;if(!config.enabled)return;const always=reply.statusCode>=400||route==="/v1/chat";if(!always&&Math.random()>config.successSampleRate)return;await new RequestMetricsRepository(db).record({storeId,requestId:request.id,method:request.method,route,statusCode:reply.statusCode,latencyMs:Math.max(0,Math.round(reply.elapsedTime)),...(reply.statusCode>=500?{errorCode:`HTTP_${reply.statusCode}`}:{})})}catch(error){request.log.warn({err:error},"Request telemetry recording failed")}finally{await close()}});
  ...
  app.get("/v1/admin/stores/:storeId/quality-analytics",async(request,reply)=>{if(!authorized(request))return reply.code(401).send({error:"Unauthorized"});const p=z.object({storeId:z.string().min(1)}).safeParse(request.params),query=z.object({days:z.coerce.number().int().min(1).max(90).default(7)}).safeParse(request.query);if(!p.success||!query.success)return reply.code(400).send({error:"Invalid analytics query"});const{db,close}=createDatabase();try{const config=await new StoreConfigurationRepository(db).resolve(p.data.storeId),observability=config?.observability??defaultObservability;const[analytics,usage,operations]=await Promise.all([new QualityAnalyticsRepository(db).dashboard(p.data.storeId,query.data.days),new AiUsageRepository(db).summary(p.data.storeId),new RequestMetricsRepository(db).dashboard(p.data.storeId,query.data.days,observability)]);return{analytics,usage,operations,observability}}finally{await close()}});
  ...
  app.post("/v1/admin/stores/:storeId/quality-scenarios/run-all",async(request,reply)=>{if(!authorized(request))return reply.code(401).send({error:"Unauthorized"});const p=z.object({storeId:z.string().min(1)}).safeParse(request.params);if(!p.success)return reply.code(400).send({error:"Invalid store"});const{db,close}=createDatabase();try{const repo=new QualityRepository(db),scenarios=(await repo.list(p.data.storeId)).filter(scenario=>scenario.enabled),runs=[];for(const scenario of scenarios){const replay=await app.inject({method:"POST",url:"/v1/chat",payload:{storeId:p.data.storeId,message:scenario.message}});const response=replay.json() as Record<string,unknown>,expected=qualityExpectationsSchema.parse(scenario.expectations),failures=evaluateResponse(response,expected);runs.push(await repo.record(scenario.id,!failures.length,failures,response));}return{summary:{total:runs.length,passed:runs.filter(run=>run.passed).length,failed:runs.filter(run=>!run.passed).length},runs}}finally{await close()}});
